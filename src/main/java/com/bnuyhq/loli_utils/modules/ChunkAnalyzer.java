...
import com.bnuyhq.loli_utils.utility.ChunkUtils;
...
public class ChunkAnalyzer extends Module {
    public final ObjectBigArrayBigList<ChunkPos> checkBlocksChunks = new ObjectBigArrayBigList<>(300000);
    public final ObjectBigArrayBigList<ChunkPos> blockUpdateChunks = new ObjectBigArrayBigList<>(300000);
    ...
    public final Setting<Boolean> preRender = sgOutlines.add(new BoolSetting.Builder()
        .name("pre-render-chunks")
        .description("Shows the render for chunk pre-render of another outline.")
        .defaultValue(true)
        .build());
    public final Setting<Boolean> checkBlocks = sgOutlines.add(new BoolSetting.Builder()
        .name("check-blocks")
        .description("Checks for certain blocks, designed for 1.12 updated to newer versions.")
        .defaultValue(true)
        .build());
    public final Setting<List<Block>> blockList = sgOutlines.add(new BlockListSetting.Builder()
        .name("blocks")
        .description("Blocks to check for.")
        .defaultValue(Blocks.NETHER_GOLD_ORE, Blocks.COPPER_ORE)
        .build());
    public final Setting<Boolean> blockUpdates = sgOutlines.add(new BoolSetting.Builder()
        .name("block-updates")
        .description("Check for Liquid/Falling block updates.")
        .build());
    public final Setting<Integer> blockUpdatesMin = sgOutlines.add(new IntSetting.Builder()
        .name("min-updates")
    ...
    public final Setting<Boolean> notifyOnRD = sgLogic.add(new BoolSetting.Builder()
    ...
    public final Setting<Integer> cacheSize = sgCache.add(new IntSetting.Builder()
        .name("cache-size")
        .description("The maximum cache size.")
        .sliderRange(128, 3000000)
        .defaultValue(1024)
        .build());
    public final Setting<Boolean> cullByDistance = sgCache.add(new BoolSetting.Builder()
        .name("cull-by-distance")
        .description("Culls chunks by Chebyshev Distance from the player.")
        .defaultValue(false)
        .build());
    public final Setting<Boolean> autoAdjust = sgCache.add(new BoolSetting.Builder()
        .name("auto-adjust")
        .description("Automatically adjusts the distance based on load distance.")
        .defaultValue(false)
        .visible(cullByDistance::get)
        .build());
    public final Setting<Integer> cullDistance = sgCache.add(new IntSetting.Builder()
        .name("cull-distance")
        ...
        .build());
    public final Setting<Boolean> clearOnLeave = sgCache.add(new BoolSetting.Builder()
        .name("clear-on-leave")
        .description("Clears caches when you leave the game.")
        .defaultValue(false)
        .build());
    public final Setting<Boolean> clearOnToggle = sgCache.add(new BoolSetting.Builder()
    ...
    public final Setting<RenderMode> renderMode = sgRender.add(new EnumSetting.Builder<RenderMode>()
        ...
        .build());
    public final Setting<Double> bouncePeak = sgRender.add(new DoubleSetting.Builder()
        .name("bounce-height")
        ...
        .defaultValue(3.5)
        .build());
    public final Setting<Double> bounceRate = sgRender.add(new DoubleSetting.Builder()
        .name("bounce-rate")
        ...
        .defaultValue(0.085)
        .build());
    public final Setting<SettingColor> checkColor = sgRender.add(new ColorSetting.Builder()
        .name("check")
        .defaultValue(new Color(187, 115, 236, 255))
        .build());
    public final Setting<SettingColor> checkStrokeColor = sgRender.add(new ColorSetting.Builder()
        .name("check-stroke")
        .visible(() -> renderMode.get() == RenderMode.Stroke)
        .defaultValue(new Color(187, 115, 236, 255))
        .build());
    public final Setting<Integer> checkYStart = sgRender.add(new IntSetting.Builder()
        .name("check-y")
        .description("Y Level to start rendering check-blocks the outline.")
        .sliderRange(-64, 320)
        .defaultValue(-58)
        .build());
    public final Setting<SettingColor> updateColor = sgRender.add(new ColorSetting.Builder()
        .name("update")
        .defaultValue(new Color(55, 112, 234, 255))
        .build());
    public final Setting<SettingColor> updateStrokeColor = sgRender.add(new ColorSetting.Builder()
        .name("update-stroke")
        .visible(() -> renderMode.get() == RenderMode.Stroke)
        .defaultValue(new Color(55, 112, 234, 255))
        .build());
    public final Setting<Integer> liquidYStart = sgRender.add(new IntSetting.Builder()
        .name("update-y")
        .description("Y Level to start rendering the block-updates outline.")
        .sliderRange(-64, 320)
        .defaultValue(-64)
        .build());
    public final Setting<SettingColor> preColor = sgRender.add(new ColorSetting.Builder()
        .name("pre")
        .defaultValue(new Color(255, 59, 59, 255))
        .build());
    public final Setting<SettingColor> preStrokeColor = sgRender.add(new ColorSetting.Builder()
        .name("pre-stroke")
        .visible(() -> renderMode.get() == RenderMode.Stroke)
        .defaultValue(new Color(255, 59, 59, 255))
        .build());
    public final Setting<Integer> preYStart = sgRender.add(new IntSetting.Builder()
        .name("pre-y")
    ...
    public void onPacketReceived(PacketEvent.Receive event) {
        if (event.packet instanceof ChunkDataS2CPacket packet) {
            add(packet.getX(), packet.getZ(), ChunkType.PRE);
        }
    ...
    public void onTick(TickEvent.Pre event) {
        ...
        if (cullByDistance.get()) {
            ...
            checkBlocksChunks.removeIf(pos -> pos.getChebyshevDistance(mc.player.getChunkPos()) >= getCullDistance());
            blockUpdateChunks.removeIf(pos -> pos.getChebyshevDistance(mc.player.getChunkPos()) >= getCullDistance());
            preChunks.removeIf(pos -> pos.getChebyshevDistance(mc.player.getChunkPos()) >= getCullDistance());
        }
    }
        ...
        try {
            for (ChunkPos pos : checkBlocksChunks) {
                renderChunk(renderer,
                    pos.getStartX(), checkYStart.get(), pos.getStartZ(),
                    pos.getEndX(), checkYStart.get() + 2, pos.getEndZ(),
                    ChunkType.CHECK_BLOCKS);
            }

            for (ChunkPos pos : blockUpdateChunks) {
                renderChunk(renderer,
                    pos.getStartX(), liquidYStart.get(), pos.getStartZ(),
                    pos.getEndX(), liquidYStart.get() + 2, pos.getEndZ(),
            ...
            for (ChunkPos pos : preChunks) {
                ...
                renderChunk(renderer,
                    pos.getStartX(), preYStart.get(), pos.getStartZ(),
                    pos.getEndX(), preYStart.get() + 2, pos.getEndZ(),
                    ChunkType.PRE);
            }
    ...
    public void add(int x, int z, ChunkType type) {
        try {
            ...
            switch (type) {
                case PRE -> {
                    if (preChunks.contains(pos) || checkBlocksChunks.contains(pos) || blockUpdateChunks.contains(pos) || entityIdChunks.contains(pos))
                        return;
            ...
            LoliUtils.LOG.error("An exception was thrown while updating chunks.\n", e.getCause());
        }
    }

    public void renderChunk(Renderer3D renderer, double x1, double y1, double z1, double x2, double y2, double z2, ChunkType type) {
        ...
        switch (renderMode.get()) {
            case Static -> {
                renderer.boxLines(x1, y1, z1,
                    x2, y2, z2,
                    color, 0);
                color.a(60);
                renderer.boxSides(x1, y1, z1,
                    x2, y2, z2,
                    color, 0);
                color.a(origAlpha);
            }
            ...
            case Bounce -> {
                renderer.boxLines(x1, y1 + renderOffset, z1,
                    x2, y2 + renderOffset, z2,
                    color, 0);
                color.a(60);
                renderer.boxSides(x1, y1 + renderOffset, z1,
                    x2, y2 + renderOffset, z2,
                    color, 0);
                color.a(origAlpha);
            }
            ...
            case Stroke -> {
                ...
                renderer.boxSides(x1, y1, z1,
                    ...
                    color, 0);
                color.a(140);
                renderer.boxLines(x1 - 0.15, y1 + renderOffset, z1 + 0.15,
                    x2 + 0.15, y1 + renderOffset + 1, z2 + 0.15,
                    switch (type) {
